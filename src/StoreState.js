...
import store_array_proxy from "./StoreArrayProxy.js";

export function is_store_object(object) {
    return typeof object === 'object' && 'store' in object && object.store instanceof StoreState;
}
...
class StoreMapProxy {
    ...
    static create_from_object(object, parent_store, object_name, actions={}) {
        const proxy_handler = new StoreMapProxy(object, parent_store, object_name, actions);
        const proxy = new Proxy(object, proxy_handler);
        proxy_handler.store.object = proxy;
        return proxy;
    }

    constructor(object, parent_store, object_name, actions) {
        this.object = object;
        this.store = new StoreState(object_name, parent_store);
...
export class StoreState {
    constructor(object_name, parent_store=null, is_part_of_url=true) {
        this.object_name = object_name;
        this.parent_store = parent_store || global_store;
        ...
        if (parent_store == null) {
            const root_var = global_store.get(this.object_name);
            if (root_var == null)
                global_store._register_store(this.object_name, this);
            else
                this.parent_store = root_var.store;
        }
    }
    ...
    get_object_path() {
        if (this.object_name != null) {
            if (this.parent_store != null)
                return `${this.parent_store.get_object_path()}/${this.object_name}`;
            return `${this.object_name}`;
        }
        if (this.parent_store != null)
            return this.parent_store.get_object_path();
        return ``;
    }
    ...
    get(path) {
        ...
        if (this.state[path_parts[0]] instanceof StoreMap) {
            ...
            if (next_path_parts[1] == null) {
                ...
                if (next_path_parts[0] in this.state[path_parts[0]])
                    return this.state[path_parts[0]][next_path_parts[0]].store.get(next_path_parts[1]);
            }
        }
        else if ('store' in this.state[path_parts[0]] && this.state[path_parts[0]].store instanceof StoreState) {
            return this.state[path_parts[0]].store.get(path_parts[1]);
        }
    ...
    set(path, object) {
        ...
        if (path_parts[1] == null) {
            if (this.state[path] instanceof StoreMap) {
                ...
                this.state[path] = this._create_object_map(object, this.state[path], this, path, this.actions[path]);
        ...
        if (object != null) {
            ...
            Object.defineProperty(object, property_name, {
                ...
                set: function (new_value) {
                    ...
                    this.store.state[property_name] = create_new_value(new_value, this.store.state[property_name], this.store, property_name, actions);
    ...
    _set_object_map(object, property_name, object_list, query_string=null, actions={}, on_change_callback=null) {
        const object_map = this._create_object_map(object_list, null, this, property_name, actions);
    ...
    _create_object_map(object_list, current_object_map=null, parent_store, property_name, actions) {
        if (!Array.isArray(object_list))
            throw new Error("Expected an array of objects");

        const object_map = StoreMapProxy.create_from_object(new StoreMap(), parent_store, property_name, actions);
